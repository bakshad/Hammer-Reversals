...
FNO_SYMBOLS = [
    ...
    "BANKBARODA.NS", "BEL.NS", "BERGEPAINT.NS", "BHARATFORG.NS", "BHARTIARTL.NS", "BHEL.NS", "BIOCON.NS", "BOSCHLTD.NS", 
    ...
    "IRCTC.NS", "ITC.NS", "JINDALSTEL.NS", "JKCEMENT.NS", "JSWSTEEL.NS", "JUBLFOOD.NS", "KOTAKBANK.NS", "LALPATHLAB.NS", 
    ...
    "NAVINFLUOR.NS", "NESTLEIND.NS", "NMDC.NS", "NTPC.NS", "OBEROIRLTY.NS", "ONGC.NS", "PAGEIND.NS", "PERSISTENT.NS", 
...
def get_signal(symbol, memory):
    try:
        ...
        # We look at the LAST COMPLETED candle (index -2)
        row = df.iloc[-2]
        ...
        row = df.iloc[-2]
        vol_ok = row['Volume'] > row['Vol_SMA10']
        rsi_val = row['RSI']
        
        # Pattern Logic + Trend Strength
        is_hammer = (row['Lower_Shadow'] > row['Body'] * 2) and (row['Upper_Shadow'] < row['Body'] * 0.5) and (row['Close'] < row['SMA20']) and vol_ok and (rsi_val < 35)
        is_star = (row['Upper_Shadow'] > row['Body'] * 2) and (row['Lower_Shadow'] < row['Body'] * 0.5) and (row['Close'] > row['SMA20']) and vol_ok and (rsi_val > 65)
        ...
        if is_hammer or is_star:
            ...
            msg = (f"🎯 *{type_str}*\nStock: `{symbol.split('.')[0]}`\n"
                   f"🔥 RSI: {rsi_val:.2f} ({'Oversold' if rsi_val < 35 else 'Overbought'})\n"
                   f"Time: {last_ts}\n"
                   f"---------------------------\n"
